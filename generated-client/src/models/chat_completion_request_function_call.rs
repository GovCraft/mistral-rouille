...
pub struct ChatCompletionRequestFunctionCall {
    ...
    #[serde(rename = "model")]
    pub model: String,
    /// The prompt(s) to generate completions for, encoded as a list of dict with role and content. The first prompt role should be `user` or `system`. When role is `tool`, the properties should contain `tool_call_id` (string or `null`).
    #[serde(rename = "messages")]
    pub messages: Vec<models::ChatFnMessages>,
    ...
    pub tools: Option<Vec<models::ChatCompletionRequestFunctionCallToolsInner>>,
...
impl ChatCompletionRequestFunctionCall {
    /// Creates a new `ChatCompletionRequestFunctionCall` instance.
    ///
    /// # Arguments
    /// * `model`
    /// * `messages`
    ...
    /// # Returns
    ///
    /// A new `ChatCompletionRequestFunctionCall` instance.
    pub fn new(
        model: String,
        messages: Vec<models::ChatFnMessages>,
